...
import com.example.demo.iface.filter.AuthorityHandlerFilterFunction;
import com.example.demo.iface.filter.function.JwtHandlerFilterFunction;
...
public class RouteConfig {
	...
	public RouterFunction<ServerResponse> routes(UserHandler handler) {
		return RouterFunctions.route(RequestPredicates.GET("/api/v1/users"), handler::getUserList)
				...
				.filter(new JwtHandlerFilterFunction(jwTokenManager)); // JWToken Filter
	...
	public RouterFunction<ServerResponse> routesForAuth(AuthHandler handler) {
		return RouterFunctions.route(RequestPredicates.POST("/api/v1/auth"), handler::grantAuth)
				.andRoute(RequestPredicates.POST("/api/v1/auth/validate"), handler::validateToken)
				.filter(new JwtHandlerFilterFunction(jwTokenManager));
	}
