...
public class CreateRoleClient {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		// 新增 請求
		CreateRoleResource resource = new CreateRoleResource(null, "主管", "USER", "系統管理員");
		
		Mono<String> resultForAdd = client.post().uri("/roles").bodyValue(resource).retrieve() // 準備檢索響應。這一步配置了請求以便後續可以獲取到響應。
				.bodyToMono(String.class); // 將響應體解析為 Mono<String>。
		resultForAdd.subscribe(response -> System.out.println("Response Data For Creating Role : " + response));
