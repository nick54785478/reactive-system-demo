...
public class QueyRoleClient {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		// 將響應體解析為 Mono<String>。
		// 刪除 請求
		Mono<String> resultForDelete = client.get().uri("/roles/{id}", 1L).retrieve().bodyToMono(String.class);
		// 訂閱並打印結果
		resultForDelete.subscribe(response -> {
			System.out.println("Response Data For Queried Role: Role" + response);
		});
