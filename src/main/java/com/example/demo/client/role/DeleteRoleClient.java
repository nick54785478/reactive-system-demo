...
public class DeleteRoleClient {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		// 刪除 請求
		Mono<String> response = client.delete().uri("/roles/{id}", 2L).retrieve().bodyToMono(String.class);
		// 等待所有訂閱完成
		// 訂閱並打印結果
		response.subscribe(responseData -> {
			System.out.println("Response Data For Deleted Role: Role" + responseData);
		});
		latch.await();
